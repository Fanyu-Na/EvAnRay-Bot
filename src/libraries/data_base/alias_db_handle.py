...
class Alias(object):
    ...
    def searchMusic(self,alias: str):
    ...
    def addalias(self,user_id:str,group_id: str,music_id: str,alias: str,songTitle: str):
        if music_id in self.searchMusic(alias):
            return "该别名已经存在", 0

        if list(self.alias_examine_collection.find({"music_id":music_id,"alias":alias})):
            return '该别名已经存在', 0

        uncount = self.alias_examine_collection.find({"$where": "this.agreeList.length < 3"}).count()
        if uncount >= 35:
        ...
        update_result = self.alias_examine_collection.update_many(
            {
                ...
                "$where": "this.agreeList.length < 3"
            },
            {
                "$set": {
                    "agreeList": ["别名处理","申请时间","超时"]
                }
            }
        )

        remove_count = update_result.modified_count
        examine_data = self.alias_examine_collection.find({"$where": "this.agreeList.length < 3"})
